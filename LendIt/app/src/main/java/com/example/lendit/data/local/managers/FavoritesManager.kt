...
class FavoritesManager(
    ...
    suspend fun refresh(userId: Int) {
        val favRecords = favoriteRepo.getFavorites(userId)
        val favorites  = favRecords.mapNotNull { listingRepo.getListingById(it.listingId) }
        ...
        similar = if (favorites.isNotEmpty()) {
            ...
            listingRepo.getListingsByCategories(categories)
